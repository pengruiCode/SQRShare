...
@property (nonatomic,strong) NSString *miniAppPath;             //跳转小程序的页面层级路径
...
- (void)setView {
    ...
    //计算分享视图的总高度
    float f = _platformArray.count/4.0;
    self.shreViewHeight = heightSpace *(self.platformArray.count /4 + 2) + buttonHeight * ceilf(f) + cancelHeight;
...
- (void)shareParamsTitle:(NSString *)title
    ...
    //    //压缩图片
    //    UIImage *imagef = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:@""]]];
    //    UIImage *newImage = [SQRCommonFunction compressImage:imagef withSize:CGSizeMake(100, 100)];
    
    _shareParams = [NSMutableDictionary dictionary];
    self.shareUrl = url;
    self.miniAppPath = path;
...
- (void)clickShare:(UIButton *)sender {
    switch (sender.tag) {
        ...
        case PRShareTypeWechatMiniApp://微信小程序
        {
            _shareType = SSDKPlatformSubTypeWechatSession;
            if (self.miniAppPath.length) {
                NSDictionary *dict = _shareParams;
                [_shareParams SSDKSetupWeChatParamsByTitle:dict[@"title"]
                                               description:dict[@"text"]
                                                webpageUrl:[NSURL URLWithString:@"http://api.shequren.cn/fcb/"]
                                                      path:self.miniAppPath
                                                thumbImage:dict[@"images"]
                                                  userName:@"gh_ad7ab54d00f3"
